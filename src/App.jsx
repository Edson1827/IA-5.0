...
const ContractModal = ({ isOpen, onClose, plan }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Calcular preço total com upsells
    let totalPrice = plan?.price || 0
    if (formData.salesCampaign) totalPrice += 97
    if (formData.brandCampaign) totalPrice += 77
    
    // Simular envio (aqui você integraria com seu backend)
    const message = `Olá! Gostaria de contratar o plano ${plan?.name}:
...
💰 *Plano Selecionado:*
• ${plan?.name}: 12x de R$ ${plan?.price}
${formData.salesCampaign ? '• Campanha de vendas: +R$ 97/mês' : ''}
${formData.brandCampaign ? '• Campanha de reconhecimento: +R$ 77/mês' : ''}

💵 *Total: 12x de R$ ${totalPrice}*

Aguardo contato para finalizar a contratação! 🚀`
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="border-t pt-4">
              <h4 className="font-semibold mb-3 dark:text-white">Turbine seus resultados</h4>
              ...
              <div className="space-y-3">
                <label className="flex items-start space-x-3 cursor-pointer">
                  <input
                    type="checkbox"
                    name="salesCampaign"
                    checked={formData.salesCampaign}
                    onChange={handleInputChange}
                    className="mt-1"
                  />
                  <div className="flex-1">
                    <div className="font-medium dark:text-white">Campanha de vendas</div>
                    <div className="text-sm text-gray-600 dark:text-gray-300">
                      Potencialize seus resultados
                    </div>
                    <div className="text-sm font-semibold text-primary">+R$ 97/mês</div>
                  </div>
                </label>
                ...
                <label className="flex items-start space-x-3 cursor-pointer">
                  <input
                    type="checkbox"
                    name="brandCampaign"
                    checked={formData.brandCampaign}
                    onChange={handleInputChange}
                    className="mt-1"
                  />
                  <div className="flex-1">
                    <div className="font-medium dark:text-white">Campanha de reconhecimento de marca</div>
                    <div className="text-sm text-gray-600 dark:text-gray-300">
                      Potencialize seus resultados
                    </div>
                    <div className="text-sm font-semibold text-accent">+R$ 77/mês</div>
                  </div>
                </label>
              </div>
            </div>
...
const WhatsAppFloat = () => {
  ...
  return (
    <button
      ...
      className="fixed bottom-4 right-4 z-50 bg-green-500 hover:bg-green-600 text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 animate-pulse sm:bottom-6 sm:right-6 sm:p-4"
